...
def janela2qualidades(prioridade):
	...
	if selfAddon.getSetting('fonte-auto') == "false":
		...
		if prioridade == "1":
			...
			if selfAddon.getSetting('host2') == "Opção 1":
				save(datapath + "option.txt","1")
			if selfAddon.getSetting('host2') == "Opção 2" or selfAddon.getSetting('host2') == "Opção 3":
				save(datapath + "option.txt","2")			
			return
		if prioridade == "2":
			try: save(datapath + "option.txt","")
			...
			if selfAddon.getSetting('host3') == "Opção 1":
				save(datapath + "option.txt","1")
			if selfAddon.getSetting('host3') == "Opção 2" or selfAddon.getSetting('host3') == "Opção 3":
				save(datapath + "option.txt","2")			
			return
...
def janela3qualidades(prioridade):
	if selfAddon.getSetting('fonte-auto') == "false":
		...
		if prioridade == "1":
			...
			if selfAddon.getSetting('host2') == "Opção 1":
				save(datapath + "option.txt","1")
			if selfAddon.getSetting('host2') == "Opção 2":
				save(datapath + "option.txt","2")
			if selfAddon.getSetting('host2') == "Opção 3":
				save(datapath + "option.txt","3")			
			return
		if prioridade == "2":
			try: save(datapath + "option.txt","")
			...
			if selfAddon.getSetting('host3') == "Opção 1":
				save(datapath + "option.txt","1")
			if selfAddon.getSetting('host3') == "Opção 2":
				save(datapath + "option.txt","2")
			if selfAddon.getSetting('host3') == "Opção 3":
				save(datapath + "option.txt","3")			
			return
...
def menu_pedidos(url):
	...
	print match
	try:
		addDir_reg_menu("[COLOR green]Página " + str(pag_actual) +"/"+str(pag_total) + " |[B] Seguinte >>[/COLOR][/B]",'http://www.ratotv.net/requests/page/' + str(int(pag_actual)+1) + '/',33,artfolder+'seta.jpg',True,fanart=fanart_rato_tv)
	except: pass
	pedidos_view()
...
def listar_seguir():
	...
	if dircontents:
		...
		while i < len(dircontents):
			...
			i +=1
		homepage_view()
	else: mensagemok('RatoTV','Não está a seguir nenhuma série.'),sys.exit(0)
	...
	if html_source:
		...
		while int(current_page) < int(total_paginas):
			...
			for trunk in html_source_trunk:
				...
				if comando == 'todos':
					progresso.update(int((float(current_page)/int(total_paginas)*100)),'A transferir biblioteca de Filmes...Aguarde...',infolabels['originaltitle'] + ' (' + infolabels['Year'] +')','Página '+str(current_page)+'/'+str(total_paginas))
